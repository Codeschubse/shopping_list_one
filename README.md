...
So we write our shopping list according to the positioning of the product groups inside the store. And as an exception and unplanned the next time we go shopping, it's in another store because it's conveniently sited on our way home from our field work that day. Of course in that store the positioning of the product groups is different to our usual store of choice. So our shopping list is in the wrong order, again.
...
You just write down the stuff you need to buy in oneShoppingList. Yes, all of it. Yes, in the same one shopping list. And you don't mind the order of the items either.

oneShoppingList does that for you, also.

In order to do so, you create a product-group-order-in-store list in the app beforehand for the stores that you visit. Then oneShoppingList will sort all items in your list depending on the store that you're about to visit in the proper order. In doing so, oneShoppingList will not only take into account, which product groups there are in the store, but also the order of their positioning inside the store, so your list is always sorted correctly without beeing unnecessarily confusing, since it won't show items that aren't sold in that store anyway. (It will show them but separately and with a corresponding notice.)
...
Record all the product groups that come to your mind when you envision going through all your favourite stores. This will create a list of product groups in alphabetical order.

Product groups can be edited but in order to not break any positioning or shopping lists only unused product groups can be deleted.

### Items

For a future version of the app it is planned be possible to record new items and product groups »on the fly« while writing your shopping list on the start page but for now, all items that you want to put on the shopping list must be recorded here first.

Like product groups, items can be edited and deleted, too. The latter again only as long as they are unused on any shopping list.
...
#### Edit product group order

Finally we're getting to the gist of the matter – the product group positionings likely differing from store to store.

You'll find an _edit product group order_ button next to each store listed on the stores page. Press it and the app will show the positioning order for the selected store. There you can choose from all the product groups, that you've recorded previously, and add them to the product groups available in this particular store.

Of course you also can rearrange the order if necessary and also delete product groups in the store's positionings.
...
The surprisingly honest answer is, it doesn't. It makes no practical sense, that is. This app started as my final project in the HarvardX course Introduction in Computer Science and initially wasn't meant to be used afterwards in an actual shop.

I may or may not port the app to Android (and iOS) sometime in the future.

Also I as respectfully as gratefully somewhat copypasted the session code from cs50's finance problem set. I'm convinced if the staff at Harvard University had considered that code to be inferior they would not have us students and course participants use it in the first place.
...
Then fate struck twice. First merciful (2 children) then hard (severe illness). I had to realign not only my career but my whole life (to the better, in retrospect). I worked in retail and later as a truck driver as an interim solution until I now eventually got the chance to again become a programmer. Thanks for your interest in my app and me and please bear with me since english is not my native language.
